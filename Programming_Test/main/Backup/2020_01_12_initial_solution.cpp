...
void updateinitialsolution(stop* route[],int n,node* list,const int num_of_terminal,const int num_of_destination, int* terminal_list,int* destination_list, const int R_max, const double T_max,const int S_max)
{
	...
	// To ensure all nodes in TSW are present
	int *check2;
	check2 = new int[23];
	...
	stop *temp;
	stop *temp2;
	double time_sum, time1, time2, shortest_time_sum,shortest_time1,shortest_time2;
	...
	for (int i=1;i<= 23;i++)
	{
		if (check2[i] == 0)
		{
			...
			for (int j=1; j<=length(route[x])-2 ;j++)
			{
				...
				time1 = Dijkstra(list,n,temp2->index,i);
				time_sum += time1;
				temp2 = temp2 -> next;
				time2 = Dijkstra(list,n,i,temp2->index);
				time_sum += time2;

				temp = temp -> next;
				for (int k=j; k<length(route[x])-1;k++)
					time_sum += Sumandnextstop(temp);
			...
			InsertAfter(pos,route[x],i,shortest_time1,shortest_time2);
			check[i]++;
		}
	}
