...
struct node
{
	double shortest; // This variable seems useless
	int pred;
	bool flag;
	path link[7];
};
...
void initialsolution(int count, stop* route[], int n, node* list,const int a,const int b,int*& terminal_list, int*& destination_list, const int R_max, const double T_max, const int S_max);
...
void updateinitialsolution(stop* route[],int n,node* list,const int num_of_terminal,const int num_of_destination, int* terminal_list,int* destination_list, const int R_max, const double T_max,const int S_max);
...
int main()
{
	...
	for (int i=1;i<=R_max;i++)
		route[i] = new stop;
	initialsolution(count,route,n,mylist,num_of_terminal,num_of_destination,terminal_list,destination_list,R_max,T_max,S_max);
	...
	cout << endl << endl;
	updateinitialsolution(route,n,mylist,num_of_terminal,num_of_destination,terminal_list,destination_list,R_max,T_max,S_max);
...
void updateinitialsolution(stop* route[],int n,node* list,const int num_of_terminal,const int num_of_destination, int* terminal_list,int* destination_list, const int R_max, const double T_max,const int S_max)
{
	...
	while (add != -1)
	{
		for (int i=1;i<= R_max;i++)
		{
			...
			if (ptr->index == key)
			{
				...
				t = Dijkstra(list, n, ptr2->index, add);
				del(route[i],key);
	...
	for (int i=1;i<= R_max;i++)
	{
		...
		t1 = Dijkstra(list,n,ptr2->index,29);
		t2 = Dijkstra(list,n,29,ptr->index);
